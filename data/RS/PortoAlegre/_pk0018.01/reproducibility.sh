...
mkdir -p /tmp/pg_io
wget -P /var/www/dl.digital-guard.org https://dl.digital-guard.org/fbc62efdbd0f16a7db03b5b775c50c9425367a8dee8a85d9b53e4475bbdb99d2.zip
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952084,'carlos',952084,'+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952084"
cd /tmp/sandbox/_pk7600001801_001; shp2pgsql -D   -s 952084 "NRO_IMOVEL.shp" pk7600001801101_p1_geoaddress | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
psql postgres://postgres@localhost/ingest1 -c "DROP  TABLE IF EXISTS pk7600001801101_p1_geoaddress CASCADE"
@echo "Delete SRID 952084 configurado via PROJ.4 string:"
@echo "+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
psql postgres://postgres@localhost/ingest1 -c "DELETE FROM spatial_ref_sys WHERE srid=952084;"
...
mkdir -m 777 -p /tmp/sandbox/_pk7600001801_001
mkdir -p /tmp/pg_io
wget -P /var/www/dl.digital-guard.org https://dl.digital-guard.org/258772459588f7941ac6eeda7c6839df02d58741cf6bc3e48825aecd60418251.zip
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952084,'carlos',952084,'+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952084"
cd /tmp/sandbox/_pk7600001801_001; shp2pgsql -D   -s 952084 "Bairros_LC12112_16.shp" pk7600001801201_p2_nsvia | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
mkdir -m 777 -p /tmp/sandbox/_pk7600001801_001
mkdir -p /tmp/pg_io
wget -P /var/www/dl.digital-guard.org https://dl.digital-guard.org/c35e3b2825d3da122a74c6c8dd5f3157a3bd6201208d2a6d13dec433a6ded9fc.zip
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952084,'carlos',952084,'+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952084"
cd /tmp/sandbox/_pk7600001801_001; shp2pgsql -D   -s 952084 "EixosLogradouros.shp" pk7600001801301_p3_via | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
