...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952049,'carlos',952049,'+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952049"
cd /tmp/sandbox/_pk7600001801_001; shp2pgsql -D   -s 952049 "NRO_IMOVEL.shp" pk7600001801101_p1_geoaddress | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
psql postgres://postgres@localhost/ingest1 -c "DROP  TABLE IF EXISTS pk7600001801101_p1_geoaddress CASCADE"
@echo "Delete SRID 952049 configurado via PROJ.4 string:"
@echo "+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
psql postgres://postgres@localhost/ingest1 -c "DELETE FROM spatial_ref_sys WHERE srid=952049;"
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952049,'carlos',952049,'+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952049"
cd /tmp/sandbox/_pk7600001801_001; shp2pgsql -D   -s 952049 "Bairros_LC12112_16.shp" pk7600001801201_p2_nsvia | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952049,'carlos',952049,'+proj=tmerc +lat_0=0 +lon_0=-51 +k=0.999995 +x_0=300000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952049"
cd /tmp/sandbox/_pk7600001801_001; shp2pgsql -D   -s 952049 "EixosLogradouros.shp" pk7600001801301_p3_via | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
