...
mkdir -p /tmp/pg_io
wget -P /var/www/dl.digital-guard.org https://dl.digital-guard.org/4134127ab8fe9d96a17d1cfb833437de98a0186e7121db4994ae4763ab4d542a.rar
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952067,'carlos',952067,'+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952067"
cd /tmp/sandbox/_pk7600007101_002; shp2pgsql -D   -s 952067 "OpenStreetMap_Shapefiles/QUADRAS.shp" pk7600007101101_p1_block | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
psql postgres://postgres@localhost/ingest1 -c "DROP  TABLE IF EXISTS pk7600007101101_p1_block CASCADE"
@echo "Delete SRID 952067 configurado via PROJ.4 string:"
...
psql postgres://postgres@localhost/ingest1 -c "DELETE FROM spatial_ref_sys WHERE srid=952067;"
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952067,'carlos',952067,'+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952067"
cd /tmp/sandbox/_pk7600007101_002; shp2pgsql -D   -s 952067 "OpenStreetMap_Shapefiles/EDIFICACOES.shp" pk7600007101101_p1_building | psql -q postgres://postgres@localhost/ingest1 2> /dev/null

psql postgres://postgres@localhost/ingest1 -c "CREATE VIEW vw1_pk7600007101101_p1_building AS SELECT gid, 'yes' AS building, geom FROM $(tabname)"
psql postgres://postgres@localhost/ingest1 -c "SELECT ingest.any_load('shp2sql','/tmp/sandbox/OpenStreetMap_Shapefiles/EDIFICACOES.shp','building_none','vw1_pk7600007101101_p1_building','7600007101101','4134127ab8fe9d96a17d1cfb833437de98a0186e7121db4994ae4763ab4d542a.rar',array[]::text[],5,1)"
psql postgres://postgres@localhost/ingest1 -c "DROP VIEW vw1_pk7600007101101_p1_building"
@echo "Confira os resultados nas tabelas ingest.donated_packcomponent e ingest.feature_asis".
psql postgres://postgres@localhost/ingest1 -c "DROP  TABLE IF EXISTS pk7600007101101_p1_building CASCADE"
@echo "Delete SRID 952067 configurado via PROJ.4 string:"
@echo "+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952067,'carlos',952067,'+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952067"
cd /tmp/sandbox/_pk7600007101_002; shp2pgsql -D   -s 952067 "OpenStreetMap_Shapefiles/NUMERACAO.shp" pk7600007101101_p1_geoaddress | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952067,'carlos',952067,'+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952067"
cd /tmp/sandbox/_pk7600007101_002; shp2pgsql -D   -s 952067 "OpenStreetMap_Shapefiles/BAIRROS.shp" pk7600007101101_p1_nsvia | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
mkdir -m 777 -p /tmp/sandbox/_pk7600007101_002
mkdir -p /tmp/pg_io
wget -P /var/www/dl.digital-guard.org https://dl.digital-guard.org/92954f43c27c205f677dd707019bd34d7d47280e74d1eede9def964528671839.rar
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952067,'carlos',952067,'+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952067"
cd /tmp/sandbox/_pk7600007101_002; shp2pgsql -D   -s 952067 "OSM_Lotes/shape/Lotes.shp" pk7600007101201_p2_parcel | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
...
psql postgres://postgres@localhost/ingest1 -c "INSERT INTO spatial_ref_sys (srid, auth_name, auth_srid, proj4text, srtext) VALUES (952067,'carlos',952067,'+proj=tmerc +lat_0=0 +lon_0=-60 +k=0.999995 +x_0=400000 +y_0=5000000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs',null);"
psql postgres://postgres@localhost/ingest1 -c "SELECT srid, proj4text FROM spatial_ref_sys where srid=952067"
cd /tmp/sandbox/_pk7600007101_002; shp2pgsql -D   -s 952067 "OpenStreetMap_Shapefiles/LOGRADOUROS.shp" pk7600007101101_p1_via | psql -q postgres://postgres@localhost/ingest1 2> /dev/null
